...
class ModelTrainer:
    def __init__(self, device=torch.NoneType, config_path="configs/base_config.yaml"):
    ...
    def train_from_scratch(self, model_name, dataset_name):
        """Training model dari awal"""
        epoch_set = 5 # 5 for test runnig, can set 50 for start real train
        save_dir = self.config['save_path']['trained_models']
        print(f"\n=== Training {model_name} from scratch on {dataset_name} with epoch {epoch_set}===")
        ...
        for epoch in range(epoch_set):
            ...
            pbar = tqdm(train_loader, desc=f'Epoch {epoch+1}/{epoch_set}')
    ...
    def fine_tune_model(self, base_model_path, dataset_name):
        """Fine-tuning model pre-trained"""
        epoch_set = 2 # 2 for test, can set 20 or higer
        save_dir = self.config['save_path']['trained_models']
        print(f"\n=== Fine-tuning model on {dataset_name} with epoch {epoch_set}===")
        ...
        if 'pretrained' in base_model_path:
            ...
            # Remove fc layer weights/bias if mismatched with model
            model_state_dict = model.state_dict()
            for key in ['fc.weight', 'fc.bias']:
                if key in state_dict and key in model_state_dict:
                    if state_dict[key].shape != model_state_dict[key].shape:
                        print(f"Skipping loading parameter {key} due to size mismatch ({state_dict[key].shape} vs {model_state_dict[key].shape})")
                        state_dict.pop(key)
        ...
        for epoch in range(epoch_set):
            ...
            pbar = tqdm(train_loader, desc=f'Fine-tune Epoch {epoch+1}/{epoch_set}')
            for inputs, labels in pbar:
                inputs, labels = inputs.to(self.device), labels.to(self.device)
...
def generate_all_model_variants():
    ...
    path_config = config['save_path']['trained_models']

    all_generated_models = []
    for model_name, i in scratch_models.items():
        for dataset, i in finetune_datasets.items():
            save_path = os.path.join(path_config, f"{model_name}_{dataset}_scratch_best.pth")
            all_generated_models.append(save_path)
    ...
    for base_model in scratch_model_paths[:2]:  # Ambil 2 model sebagai contoh [:2]
        if os.path.exists(base_model):
            ...
            for target_dataset in target_datasets[:1]:  # Coba satu target dataset dulu [:1]
                try:
